...
| Roles | Multi-select | Yes | Prospect, Referrer, Related Contact |
...
**Logic:** `daysIdle >= stageThreshold AND nextActionDate <= today AND stage is active (not Nurture/Dead/Funded)`

Computed on every page load from activity data. No scheduled batch job needed — always accurate.

**Key rule:** A future Next Action Date suppresses the stale flag. Marcus Johnson at 12 days idle with a March 1 next action is NOT stale.
...
3. Committed ($) — sum of Committed Amount where stage is Soft Commit, Commitment Processing, or KYC
...
interface DataService {
  // People
  getPeople(filters?: PeopleFilters): Promise<Person[]>
...
Available from the Leadership Dashboard or as exportable views:
...
## 16. Mock Data Inventory (Ships with V1)
